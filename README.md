...
supported LLM provider (via the Vercel `ai` SDK), within a configurable step limit,
...
3. Add its API key to `envSchema` in `src/server/config.ts` and to `.env.example`.
...
From the dashboard, set `maxSteps` / `model` and click "Start run", or
...
The run stops automatically once the step limit is hit, or on demand via
