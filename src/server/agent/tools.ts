...
export const tools = {
  ...
  getShipNav: tool({
    description:
      "Get what type of navigation the ship is currently using. It can be important because of the mission type or fuel consumption.",
    inputSchema: z.object({ shipSymbol: z.string() }),
    execute: async ({ shipSymbol }) => spaceTraders.getShipNav(shipSymbol),
  }),

  updateShipNav: tool({
    description:
      "Update the ship navigation type. 'DRIFT' is for low fuel, 'STEALTH' is for stealth movement. 'BURN' is for fast navigation, but high fuel consumption. 'CURISE' is regular travel.",
    inputSchema: z.object({ shipSymbol: z.string(), flightMode: navType }),
    execute: async ({ shipSymbol, flightMode }) =>
      spaceTraders.patchShipNav(shipSymbol, flightMode),
  }),
  ...
  updateStrategy: tool({
    description:
      "Record a learning discovered during gameplay — a problem hit and how to avoid it, or an approach that worked well. Notes persist across runs and are shown to future runs to improve gameplay decisions. Keep each note short and actionable.",
    inputSchema: z.object({
      note: z.string().min(1).max(1000),
    }),
    execute: async ({ note }) => strategy.addStrategyNote(note),
  }),
  ...
  //     execute: async({ code }) => runInSandbox(code, sandboxApi),
  //   }),
};
