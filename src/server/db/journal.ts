...
export function createRun(id: string, runConfig: RunConfig): RunRecord {
  ...
  db.prepare(
    `INSERT INTO runs (id, status, max_steps, model, started_at, total_steps)
     VALUES (@id, 'running', @maxSteps, @model, @startedAt, 0)`
...
export function updateRunProgress(id: string, totalSteps: number): void {
  db.prepare("UPDATE runs SET total_steps = ? WHERE id = ?").run(totalSteps, id);
}
  ...
  const result = db
    .prepare(
      `INSERT INTO journal_entries
        (run_id, step_number, created_at, reasoning, tool_name, tool_args, tool_result, tokens_used)
       VALUES (@runId, @stepNumber, @createdAt, @reasoning, @toolName, @toolArgs, @toolResult, @tokensUsed)`
    )
    .run({
      runId: entry.runId,
